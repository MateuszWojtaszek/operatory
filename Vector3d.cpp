...
Vector3d Vector3d::operator+(const Vector3d &v) const {
    Vector3d tmp_v;
    tmp_v.pos_x = this->pos_x + v.pos_x; // once to show how it works
...
Vector3d Vector3d::operator*(const Vector3d &v) const {
   ...
   //        |a3, b3, c3| (b2*c3-b3*c2, -(a2*c3 - a3*c2),(a2*b3 - a3*b2)
    Vector3d tmp_v;
    tmp_v.pos_x=(this->pos_y*v.pos_z)-(v.pos_y*this->pos_z); //once just to show how it works
...
double Vector3d::operator[](int elem) const {
    switch (elem) {
        case 0:
            return this->pos_x;
            break;
        ...
        case 2:
            ...
            break;
        default:
            std::cout<<"KBOOM INVALID POS ACCESS "<< std::endl;
            break;
    }
    return -27548120923814;
}
